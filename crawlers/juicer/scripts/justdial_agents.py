...
class CreateCSV(Utils):
	...
	def write_data_to_file(self, source):
		try:
			...
			query = 'select agency_name, city, phone_number, address, reference_url from justdial_agents_final where created_at > DATE_ADD(NOW(), INTERVAL -1 DAY)'
			...
			for record in records:
				name, city, phone, address, reference_url = record
				if "," in phone:
					phone = "<>".join(phone.split(","))
				if re.findall('(.* \w+ - \d{6}),', address):
					address = re.findall('(.* \w+) - \d{6},', address)[0]
				data = [name, city, phone.replace(" ", ''), address.replace('(Map)', '').replace("...more", "").replace("...", '').replace("\r\r", "").replace("\t", "").strip()]
