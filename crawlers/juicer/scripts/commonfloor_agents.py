...
class CreateCSV(Utils):
	...
	def write_data_to_file(self, source):
		try:
			...
			headers = ['agent_name', 'agency_name', 'city', 'phone', 'address', 'operating_areas']
			self.data.append(headers)

			query = 'select agent_name, agency_name, city, phone_number, address, operating_areas, reference_url from commonfloor_agents where created_at > DATE_ADD(NOW(), INTERVAL -1 DAY) and operating_areas != ""'
			...
			for record in records:
				agent_name, name, city, phone, address, operating_areas, reference_url = record
				data = [agent_name.replace("&amp;", "&"), name.replace("&amp;", "&"), city.replace("%20", ' '), phone.replace(" ", ''), address.replace('(Map)', '').replace("...more", "").replace("...", '').replace("\r\r", "").replace("\t", "").strip(), operating_areas ]
	...
	def run_main(self):
		self.create_cursor()
		for source in ['commonfloor_agents']:
