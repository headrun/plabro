...
class CreateCSV(Utils):
	...
	def write_data_to_file(self, source):
		try:
			...
			headers = ['title', 'city', 'locality','latitude', 'longitude', 'bhk', 'sqft', 'furnished', 'postedBy', 'amenities', 'posted_date', 'propertyType', 'propertyTypeAvailable', 'availableFor', 'isPremium', 'dealer_name', 'email_id', 'contact_number']
			self.data.append(headers)

			query = 'select title, city, locality, latitude, longitude, bhk, sqft, furnished, postedBy, amenities, date, propertyType, propertyTypeAvailable, availableFor, isPremium, dealer_name, email_id, contact_number from quikr_properties where created_at > DATE_ADD(NOW(), INTERVAL -1 DAY)'
			...
			for record in records:
				title, city, locality, latitude, longitude, bhk, sqft, furnished, postedBy, amenities, date, propertyType, propertyTypeAvailable, availableFor, isPremium, dealer_name, email_id, contact_number = record
